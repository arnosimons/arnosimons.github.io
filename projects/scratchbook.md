...
<head>
  ...
  <script src="https://raw.githubusercontent.com/arnosimons/ttmtools/main/data.json"></script>
  <script>
    function constructTable(selector, list) {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        for (var colIndex = 0; colIndex < cols.length; colIndex++) {
          ...
          if (val == null) val = ""; // empty string instead of null
          table_row.append($('<td/>').html(val));
        }
    ...
    function Headers(list, selector) {
      ...
      for (var i = 0; i < list.length; i++) {
        var list_row = list[i];
        for (var k in list_row) {
          if ($.inArray(k, columns) == -1) {
            columns.push(k);
            table_row.append($('<th/>').html(k));
          }
        }
      }
      $(selector).append($('<thead/>').append(table_row));
      return columns;
    }
  </script>
    ...
    <div class="panel-group" id="Visualizer">
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          ...
          <input class="form-control" type="text" id="scratch" style="font-size: 14px;" value="[23,67,3]" placeholder="Type scratch formula.."/>
            ...
            <div class="card-body" style="overflow-x:auto;">
              ...
              <script>
                $(document).ready(function () {
                  $('#ScratchesTable').DataTable({
                    "order": [[ 3, "desc" ],[ 2, "desc" ]]
                  });
                    $('.dataTables_length').addClass('bs-select');
                });
              </script>
              <table class="table" id="ScratchesTable" style="font-size: 12px">
              </table>
   
      
              <script type="text/javascript">
                var mylist = [
                  { "Name": "val_11", "Link": "http//usw" },
                  { "col_2": "val_22", "col_3": "val_23" },
                  { "Name": "chirp", "Link": "smth" },
                  { "Name": "val_31", "col_39": "val_33" }
                ];
                constructTable('#ScratchesTable', mylist)
              </script>
              <br/>
  ...
  <footer class="bg-dark text-center text-white">
    ...
    <div class="container p-4">
      <section class="mb-4">
        ...
        <!-- Instagram -->
        <a class="btn btn-outline-light btn-floating m-1" href="https://www.instagram.com/arnoscratch/" target='_blank' role="button"
      ...
      <section class="mb-4">
        ...
        <p>The underlying python code can be downloaded <a href="https://github.com/arnosimons/ttmtools" target='_blank'>here</a>.</p>
      </section>
    </div>
...
<py-script>
  ...
  req.open("GET", "https://raw.githubusercontent.com/arnosimons/ttmtools/main/test.py", False)
  req.send()
  exec(str(req.response))
  <!-- pyscript.write("session-output", session([23,5,67])) -->
  def plot(x=None):
